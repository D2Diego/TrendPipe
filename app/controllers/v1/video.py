...
# Select the appropriate task manager according to configuration
if _enable_redis:
...
def _sanitize_upload_filename(filename: str, request_id: str) -> str:
    # Browser or client sometimes carries directory information, or even a folder And... ../ This kind of cross-section.
    # Only the plain filename is retained here to avoid uploading the file outside the target directory.
...
def _public_task_data(task: dict) -> dict:
    """Copys the task status and removes the internal field for service-end process coordination only."""
        ...
        bgm_list.append(
            {
                ...
                # Only return the filename to avoid exposing the absolute path of the server to the caller. Service
                # Yes. storage/bgm and resource/songs Reclassify in two white list directories.
                "file": filename,
            }
        )
...
def upload_bgm_file(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        # Upload failure can normally be restored after the user has replaced the file, so the record is recorded request_id And clear reasons,
        # But do not output file content or absolute path to avoid the log leaking user data.
        logger.warning(
        ...
        # Tool chain or storage failure is a service-end problem and cannot be disguised as a user file error. Log Retention
        # request_id And internal reasons,HTTP Responds only to stabilize the file and avoid exposing the server path.
        logger.error(
...
def get_video_materials_list(request: Request):
    ...
    # The sequence of the file system is unstable and direct return leads to a "sequence fusion" of different machines or different
    # Time is not consistent. This is done in a uniform order by file name, at least to ensure predictable service-end return sequence.
        ...
        video_materials_list.append(
            {
                "name": filename,
                "size": os.path.getsize(file),
                # and BGM Same, return only file name; on creation task local_videos
                # White List Directory parsing, avoiding API Visible host absolute path.
                "file": filename,
            }
        )
...
def upload_video_material_file(request: Request, file: UploadFile = File(...)):
    ...
    # Verify by full extension, compatible .MOV This type of capital suffix is avoided. photojpg There's no such thing.
    # The name of the file number is because endswith("jpg") It's been mistook as a legitimate picture.
