"""Safe reads and writes for persistent files in task directories."""
...
def _script_file(task_id: str) -> Path:
    """Return the task manifest path using the shared directory helper."""
...
def _write_json_atomic(target: Path, payload: Mapping[str, Any]) -> None:
    ...
    on local filesystems and Docker mounts. Clean up only this attempt on error.
    """
...
def write_script_data(task_id: str, payload: Mapping[str, Any]) -> None:
    """Create or completely replace a task's ``script.json`` manifest."""
    _write_json_atomic(_script_file(task_id), payload)


def patch_script_data(task_id: str, **updates: Any) -> bool:
    """
    Patch a task manifest while preserving existing fields; return ``False`` on failure.

    Provenance is diagnostic metadata and must not block video generation because
    of permissions, transient disk errors, or a damaged historical file.
    """
    try:
        ...
        # Tests, scripts, and third-party callers may use download_videos without
        # a task manifest; skip quietly instead of creating an incomplete file.
        logger.debug(
