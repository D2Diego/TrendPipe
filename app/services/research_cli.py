...
def ensure_supported_python(version_info: tuple[int, int, int] | object | None = None) -> None:
    ...
    sys.stderr.write(
        f"trendpipe v3 requires Python {req}+.\n"
...
from app.services.research_lib.pipeline import competitors as competitors_mod, discovery_handoff, freshness, permission_preflight, pipeline, registers
from app.services.research_lib.sources import corpus
from app.services.research_lib import dates, env, http, schema
from app.services.research_lib.output import html_render, render, ui
...
def resolve_requested_sources(args_search: str | None, config: dict) -> list[str] | None:
    ...
    default_search = (config.get("TRENDPIPE_DEFAULT_SEARCH") or "").strip()
    if default_search:
        return parse_search_flag(default_search, flag_name="TRENDPIPE_DEFAULT_SEARCH")
    return None
...
def save_output(
    ...
    for candidate in candidates:
        ...
        if candidate.suffix.lower() == ".md":
            try:
                from app.services.research_lib.output import library, library_index
                ...
                if save_root == Path(library.DEFAULT_MEMORY_DIR).expanduser().resolve():
                    ...
                    library_index.sync_library(
                        save_root,
                        save_root / "briefings",
                        db_path=save_root / ".trendpipe-library.db",
                    )
...
def publish_rendered_html(
    ...
    *,
    ...
    from app.services.research_lib.output import html_publish
...
def _publish_password_for_args(
    ...
    return (
        ...
        or env.read_secret_env("TRENDPIPE_PUBLISH_PASSWORD")
        or (config or {}).get("TRENDPIPE_PUBLISH_PASSWORD")
        or None
    )
...
def read_synthesis_file(path: str) -> str:
    try:
        ...
        sys.stderr.write(f"[trendpipe] Cannot read --synthesis-file: {exc}\n")
        raise SystemExit(2)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    freshness_group.add_argument(
        ...
        action="store_false",
        help="Disable freshness verification configured by TRENDPIPE_VERIFY_FRESHNESS",
    )
    parser.add_argument(
        "--drill",
        metavar="TARGET",
    ...
    parser.add_argument("--publish-password",
                        help="Optional shared password for --publish-html or 'library feed --publish'; prefer TRENDPIPE_PUBLISH_PASSWORD to avoid exposing secrets in process lists")
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help="Auto-discover N competitor entities and fan out trendpipe across all of them as a comparison (default N=2 → 3-way: original + 2 peers; range 1..6). Use --competitors-list to override discovery.",
    )
    parser.add_argument(
        "--competitors-list",
...
def apply_vs_competitor_routing(
    ...
    *,
    ...
    """
    from app.services.research_lib.pipeline import planner as _planner
...
REPORT_CACHE_VERSION = "trendpipe-report-cache/v1"
...
def _report_cache_ttl_seconds(config: dict[str, object]) -> int:
    raw = os.environ.get("TRENDPIPE_REPORT_CACHE_TTL_SECONDS")
    if raw is None:
        raw = config.get("TRENDPIPE_REPORT_CACHE_TTL_SECONDS")
...
def _load_last_report_cache(
    ...
    try:
        ...
        sys.stderr.write(
            f"[trendpipe] Could not read report cache {cache_path}: "
...
def _freshness_enabled(args: argparse.Namespace, config: dict[str, object]) -> bool:
    ...
    return _config_truthy(config.get("TRENDPIPE_VERIFY_FRESHNESS"))
...
def _update_cached_freshness(
    ...
    try:
        ...
        sys.stderr.write(
            f"[trendpipe] warning: could not update freshness cache: {exc}\n"
        )
        return False
...
def _verify_report_set(
    ...
    *,
    ...
    if not any(current_report.freshness_verdicts for current_report in reports):
        ...
        sys.stderr.write(
            "[trendpipe] Freshness verification found no re-checkable claims"
...
def _run_cached_freshness(
    ...
    if cached is None:
        sys.stderr.write("[trendpipe] No fresh cached report; run a research pass first.\n")
        return 2
    ...
    if _update_cached_freshness(cache_path, report, entity_reports):
        sys.stderr.write(f"[trendpipe] Updated freshness verdicts in {cache_path}\n")
    else:
        sys.stderr.write("[trendpipe] warning: freshness cache update failed\n")
...
def _run_drill(
    ...
    from app.services.research_lib.pipeline import planner
    ...
    if cached is None:
        sys.stderr.write(
            "[trendpipe] No fresh cached report; run a research pass first.\n"
        )
        return 2
    report, entity_reports, cache_path = cached
    if entity_reports:
        sys.stderr.write(
            "[trendpipe] Drill mode needs a single-topic cached report; "
    ...
    try:
        ...
        sys.stderr.write(f"[trendpipe] {exc}\n")
        return 2
    ...
    if _write_last_run(report.topic, merged):
        sys.stderr.write(f"[trendpipe] Updated drill cache in {cache_path}\n")
    else:
        sys.stderr.write(
            "[trendpipe] warning: drill cache update failed; the next drill "
            "will see the pre-drill report\n"
        )

    store_default = str(
        os.environ.get("TRENDPIPE_STORE")
        or config.get("TRENDPIPE_STORE")
        or ""
    ).lower()
    if args.store or store_default in {"1", "true", "yes"}:
        counts = persist_report(merged, store_db=_scoped_store_db(args))
        sys.stderr.write(
            f"[trendpipe] Stored {counts['new']} new, "
    ...
    if args.synthesis_file:
        if args.emit == "html":
            ...
            sys.stderr.write(
                "[trendpipe] Warning: --synthesis-file is only used with "
                "--emit=html; ignoring.\n"
            )
...
def _annotate_and_record_discovery_queue(
    ...
    default; the resolved config value TRENDPIPE_DISCOVERY_QUEUE == "off"
    ...
    """
    queue_setting = str(config.get("TRENDPIPE_DISCOVERY_QUEUE") or "").strip().lower()
...
def _record_discovery_queue_safely(
    ...
    try:
        ...
        sys.stderr.write(
            f"[trendpipe] Warning: discovery queue unavailable ({exc}); "
...
def _emit_and_save_discovery_report(
    ...
    if args.output:
        ...
        sys.stderr.write(f"[trendpipe] Saved output to {output_path}\n")
    if args.save_dir:
        ...
        sys.stderr.write(f"[trendpipe] Saved output to {save_path}\n")
    print(rendered)
...
def _discovery_strict_exit_code(
    ...
    strict = str(config.get("TRENDPIPE_STRICT_EXIT") or "").strip().lower()
    ...
    if not degraded:
        return 0
    sys.stderr.write(
        f"[trendpipe] strict-exit: degraded sources: {', '.join(degraded)}\n"
    )
    sys.stderr.flush()
    return 3
...
def _run_queue_cover(
    ...
    if not name:
        sys.stderr.write(
            "[trendpipe] queue cover requires a topic name: "
    ...
    if not Path(db_path or store.DB_PATH).exists():
        sys.stderr.write(
            f"[trendpipe] No queued topic named {name!r}: the discovery queue "
    ...
    if row is None:
        sys.stderr.write(
            f"[trendpipe] No queued topic named {name!r}. Covering requires "
...
def _resolve_discovery_source_boundary(
    ...
    if requested_sources:
        ...
        if not discovery_sources:
            origin = "--search" if args.search is not None else "TRENDPIPE_DEFAULT_SEARCH"
            sys.stderr.write(
                f"[trendpipe] {origin} has no discovery-capable sources "
...
def _run_discover(args: argparse.Namespace, config: dict[str, object]) -> int:
    ...
    if args.synthesis_file:
        sys.stderr.write("[trendpipe] Warning: --synthesis-file is not used by discovery mode.\n")
...
def _discover_handoff_state_dir(args: argparse.Namespace) -> Path | None:
    ...
    TRENDPIPE_MEMORY_DIR fallback in _main resolved it."""
...
def _run_discover_resume(args: argparse.Namespace, config: dict[str, object]) -> int:
    ...
    if state_dir is None:
        ...
        raise discovery_handoff.HandoffContractError(
            ...
            "pass --save-dir or configure ~/.config/trendpipe/."
        )
...
def _run_discover_protocol_leg(
    ...
    try:
        ...
        sys.stderr.write(f"[trendpipe] {exc.message}\n")
        return 2
...
def _strict_exit_code(
    ...
    """
    raw = str(config.get("TRENDPIPE_STRICT_EXIT") or "").strip().lower()
...
def _audience_register_for_run(
    ...
    configured = config.get("TRENDPIPE_REGISTER")
...
def _render_save_and_print(
    ...
    if audience.name != "default":
        sys.stderr.write(f"[trendpipe] Audience register: {audience.name}\n")
        sys.stderr.flush()
    ...
    if args.save_dir:
        ...
        if entity_reports and len(entity_reports) > 1:
            for label, entity_report in entity_reports[1:]:
                ...
                sys.stderr.write(f"[trendpipe] Saved output to {peer_path}\n")
            peers_display = ", ".join(str(path) for path in comparison_peer_paths)
            sys.stderr.write(
                f"[trendpipe] Comparison artifact set: main={save_path}; "
    ...
    if args.publish_html:
        try:
            ...
            if has_private_corpus:
                sys.stderr.write(
                    "[trendpipe] Excluding local corpus evidence and synthesis from published HTML.\n"
                )
                if entity_reports:
            ...
            sys.stderr.write(f"[trendpipe] Published HTML to {publish_result['url']}\n")
            for warning in publish_result.get("_metadata_errors") or []:
                sys.stderr.write(f"[trendpipe] Publish metadata warning: {warning}\n")
            if publish_result.get("update_key"):
                sys.stderr.write(
                    "[trendpipe] ht-ml.app returned an update key; not writing it "
                    "to stdout, HTML, or publish metadata.\n"
                )
            sys.stderr.flush()
        except Exception as exc:
            sys.stderr.write(f"[trendpipe] HTML publish failed: {exc}\n")
            sys.stderr.flush()
    print(rendered)
...
def _validate_extra_argv(parser: argparse.ArgumentParser, topic: str, extra_argv: list[str]) -> None:
    ...
    if skill_only:
        message = (
            ...
            + ", ".join(skill_only)
            + "; these are skill arguments and must not be forwarded to scripts/trendpipe.py"
        )
        if other_unknown:
...
def _run_library_feed(args: argparse.Namespace, config: dict[str, object]) -> int:
    ...
    from app.services.research_lib.output import feed, html_publish, library

    if args.publish_html:
        sys.stderr.write(
            "[trendpipe] library feed uses --publish, not --publish-html.\n"
        )
        return 2
    if args.output:
        sys.stderr.write(
            "[trendpipe] library feed writes index.html and feed.xml to --save-dir; "
    ...
    feed_author = str(
        config.get("TRENDPIPE_LIBRARY_OWNER") or "trendpipe research library"
    )
    ...
    def _preserve_hand_written_page(existing_path: Path, generated_marker: str) -> None:
        ...
        sys.stderr.write(
            f"[trendpipe] {existing_path.name} was not generated by "
    ...
    _preserve_hand_written_page(feed_path, "urn:trendpipe:research-library")
    _preserve_hand_written_page(
        index_path, "Generated locally by <strong>trendpipe</strong>"
    )
    feed_path.write_text(feed_xml, encoding="utf-8")
    index_path.write_text(index_html, encoding="utf-8")

    for note in notes:
        sys.stderr.write(f"[trendpipe] Library note: {note}\n")
    sys.stderr.write(
        f"[trendpipe] Library feed generated {len(entries)} brief(s): "
    ...
    if args.publish:
        ...
        try:
            ...
            sys.stderr.write(f"[trendpipe] Library publish failed: {exc}\n")
            if entry_urls:
                sys.stderr.write(
                    f"[trendpipe] Partial publish: {len(entry_urls)} public brief "
        ...
        sys.stderr.write(f"[trendpipe] Published library to {index_url}\n")
        sys.stderr.write(f"[trendpipe] Local Atom feed: {feed_path}\n")
        print(
            f"Library: {index_url}\nFeed: {feed_path}\n"
...
def _run_library_search(
    ...
    if args.publish or args.publish_html:
        sys.stderr.write("[trendpipe] library search does not publish output.\n")
        return 2
    if args.emit != "compact":
        sys.stderr.write("[trendpipe] library search currently supports text output only.\n")
        return 2
    if args.output:
        sys.stderr.write(
            "[trendpipe] library search prints to stdout; --output is not supported.\n"
        )
        return 2
    ...
    try:
        matches, synced = library_index.sync_and_search(
            ...
            db_path=(
                memory_dir.resolve() / ".trendpipe-library.db"
        ...
        sys.stderr.write(f"[trendpipe] Library search unavailable: {exc}.\n")
        return 2
    except (OSError, sqlite3.DatabaseError) as exc:
        sys.stderr.write(f"[trendpipe] Library search failed: {exc}.\n")
        return 1
    for note in synced.notes:
        sys.stderr.write(f"[trendpipe] Library note: {note}\n")
    if synced.rebuilt:
        sys.stderr.write("[trendpipe] Rebuilt a corrupt library search index.\n")
...
def _main(
    ...
    if args.welcome:
        from app.services.research_lib.auth import setup_wizard
    ...
    if args.publish and topic.lower() != "library feed":
        sys.stderr.write(
            "[trendpipe] --publish is only supported by the 'library feed' command.\n"
        )
        return 2
    ...
    resolved_corpus_dirs = corpus.resolve_directories(
        args.corpus, config.get("TRENDPIPE_CORPUS_DIRS")
    )
    ...
    if _config_truthy(config.get("TRENDPIPE_CORPUS_IN_EXPORT")):
        config["_CORPUS_IN_EXPORT"] = True
    _propagate_config_to_environ(config)

    # Env-var fallback for --save-dir, mirroring the TRENDPIPE_STORE pattern below.
    # Uses `is None` / `is not None` checks (not truthy `or`) at every layer so that
    # `--save-dir ""`, `TRENDPIPE_MEMORY_DIR=""` (shell-export-empty), and explicit
    ...
    if args.save_dir is None:
        env_val = os.environ.get("TRENDPIPE_MEMORY_DIR")
        args.save_dir = env_val if env_val is not None else config.get("TRENDPIPE_MEMORY_DIR")
    ...
    if config.get("TRENDPIPE_YOUTUBE_SSH_HOST") and "TRENDPIPE_YOUTUBE_SSH_HOST" not in os.environ:
        os.environ["TRENDPIPE_YOUTUBE_SSH_HOST"] = config["TRENDPIPE_YOUTUBE_SSH_HOST"]
    ...
    if topic.lower() == "doctor":
        from app.services.research_lib.output import doctor
        return doctor.run(
            config,
    ...
    if args.discover is not None:
        if topic:
            sys.stderr.write(
                "[trendpipe] --discover supplies the domain and cannot be combined "
                "with a positional topic.\n"
            )
            return 2
        if args.drill:
            sys.stderr.write("[trendpipe] --discover and --drill are mutually exclusive.\n")
            return 2
        ...
        if args.as_of_date:
            sys.stderr.write(
                "[trendpipe] --as-of cannot be used with --discover because discovery "
                "sweeps current live listings.\n"
            )
            return 2
        if args.emit == "html" or args.publish_html:
            sys.stderr.write("[trendpipe] discovery mode does not support HTML publishing yet.\n")
            return 2
        ...
        for first, second, conflict in (
            ...
            if conflict:
                sys.stderr.write(
                    f"[trendpipe] {first} and {second} are mutually exclusive: "
                    "each runs a different leg of the discovery protocol.\n"
                )
                return 2
        if args.angles is not None and not args.finalize:
            sys.stderr.write(
                "[trendpipe] --angles only applies to --discover --finalize "
        ...
        if protocol_leg and args.mock and not args.save_dir:
            ...
            sys.stderr.write(
                "[trendpipe] mock protocol legs require --save-dir to stay "
    ...
    if args.discover_shallow:
        ...
        sys.stderr.write(
            "[trendpipe] --discover-shallow only applies to --discover runs; "
    ...
    for flag_label, present in (
        ...
        if present:
            sys.stderr.write(
                f"[trendpipe] {flag_label} only applies to --discover runs; "
                "add --discover [domain] or drop the flag.\n"
            )
            return 2
    if args.angles is not None:
        sys.stderr.write(
            "[trendpipe] --angles only applies to --discover --finalize runs; "
    ...
    if args.drill:
        if topic:
            sys.stderr.write(
                "[trendpipe] --drill uses the cached topic and cannot be "
                "combined with a new topic.\n"
            )
            return 2
        if args.publish_html and args.emit != "html":
            sys.stderr.write("[trendpipe] --publish-html requires --emit=html\n")
            return 2
    ...
    # Remote API path: when BOTH TRENDPIPE_API_KEY and TRENDPIPE_API_BASE are
    ...
    if (
        ...
        and env.read_secret_env("TRENDPIPE_API_KEY")
        and os.environ.get("TRENDPIPE_API_BASE")
    ):
        sys.stderr.write(
            "[trendpipe] Local corpus configured; bypassing the hosted backend so files stay on this machine.\n"
        )
    if (
        topic
        and not args.diagnose
        ...
        if _freshness_enabled(args, config):
            if args.verify_freshness is True:
                sys.stderr.write(
                    "[trendpipe] Freshness verification is not supported by the hosted backend; "
        ...
        if args.emit == "json" and args.json_profile == "agent":
            sys.stderr.write(
                "[trendpipe] --json-profile=agent requires the local Report; "
                "the remote API backend only supports --json-profile=raw.\n"
            )
            return 2
        from app.services.research_lib.pipeline import hosted
        depth = "deep" if args.deep else "quick" if args.quick else "default"
        try:
    ...
    if not os.environ.get("TRENDPIPE_SKIP_PREFLIGHT"):
        from app.services.research_lib.pipeline import preflight
    ...
    if args.emit == "html" and synthesis_md is not None:
        ...
        if cached is not None:
            ...
            sys.stderr.write(
                f"[trendpipe] Reusing cached report data from {cache_path}\n"
            )
            sys.stderr.flush()
            if _freshness_enabled(args, config):
                _verify_report_set(
        ...
        sys.stderr.write(
            "[trendpipe] No matching cached report data for "
    ...
    try:
        ...
        if args.plan:
            ...
            try:
                ...
                raise SystemExit(2)
            from app.services.research_lib.pipeline import planner as _plan_validator
            try:
        ...
        if args.auto_resolve and not external_plan:
            from app.services.research_lib.pipeline import resolve
        ...
        if github_repos and not repos_from_auto_resolve:
            from app.services.research_lib.pipeline import resolve as resolve_lib
        ...
        if comp_enabled:
            from app.services.research_lib.pipeline import competitors as competitors_mod
            from app.services.research_lib.pipeline import fanout, resolve as resolve_mod
            ...
            if comp_explicit:
                ...
                if not resolve_mod._has_backend(config) and not args.mock:
                    sys.stderr.write(
                        ...
                        "  3. Re-invoke: /trendpipe '{topic} vs {peer1} vs {peer2}' "
    ...
    # TRENDPIPE_STORE env var = persistence default-on. Read both os.environ
    # (for shell-exported users) and config (for users who set it in
    # ~/.config/trendpipe/.env, which env.py loads but does not propagate
    # to os.environ). Mirrors the TRENDPIPE_DEBUG / TRENDPIPE_SKIP_PREFLIGHT
    ...
    if args.store or _store_env in ("1", "true", "yes"):
        ...
        sys.stderr.write(
            f"[trendpipe] Stored {counts['new']} new, {counts['updated']} updated findings\n"
        )
        sys.stderr.flush()
    ...
    if not args.hiring_signals:
        try:
            from app.services.research_lib.pipeline import quality_nudge
            from app.services.research_lib.sources import youtube_yt as _youtube_yt
