...
Uses yt-dlp for both YouTube search and transcript extraction. No API keys
needed — just have yt-dlp installed.
"""
...
# quality_nudge reads these via trendpipe.py to suppress the stale-yt-dlp
...
def _search_timeout() -> float:
    ...
    return _env_positive_float("TRENDPIPE_YT_SEARCH_TIMEOUT", float(_SEARCH_TIMEOUT))
...
def is_ytdlp_installed() -> bool:
    ...
    When TRENDPIPE_YOUTUBE_SSH_HOST is set, returns True without a local check —
...
def _ytdlp_ssh_host() -> Optional[str]:
    ...
    Set TRENDPIPE_YOUTUBE_SSH_HOST=<ssh-alias> (e.g. 'macmini') in the environment
    ...
    To use a value from ~/.config/trendpipe/.env, export it into the
    environment before invoking the engine, e.g. in a wrapper:
        set -a; source ~/.config/trendpipe/.env; set +a
        python3 trendpipe.py "..."
    """
    host = os.environ.get("TRENDPIPE_YOUTUBE_SSH_HOST", "").strip()
    if not host:
        return None
    if not _SSH_HOST_ALIAS_RE.match(host):
        sys.stderr.write(
            f"[youtube_yt] WARNING: TRENDPIPE_YOUTUBE_SSH_HOST={host!r} "
...
def _wrap_ytdlp_cmd(cmd: List[str]) -> List[str]:
    ...
    TRENDPIPE_YOUTUBE_SSH_HOST were ever set to a value starting with `-`.
    """
...
def _ytdlp_sub_langs() -> str:
    """Caption languages to try, from TRENDPIPE_YT_SUB_LANGS (default en,es,pt)."""
    raw = os.environ.get("TRENDPIPE_YT_SUB_LANGS", "").strip()
    if not raw:
        return "en,es,pt"
