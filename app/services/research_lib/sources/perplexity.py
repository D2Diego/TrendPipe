...
def _direct_model(config: dict, deep: bool) -> str:
    ...
    model = _config_text(config, "TRENDPIPE_PERPLEXITY_MODEL") or PERPLEXITY_MODEL_SONAR_PRO
    if model not in DIRECT_MODELS:
        _log(f"Unsupported TRENDPIPE_PERPLEXITY_MODEL={model!r}; using sonar-pro")
...
def _mode(config: dict, provider: str, deep: bool) -> str:
    ...
    mode = (_config_text(config, "TRENDPIPE_PERPLEXITY_MODE") or PERPLEXITY_MODE_SONAR).lower()
    if mode not in DIRECT_MODES:
        _log(f"Unsupported TRENDPIPE_PERPLEXITY_MODE={mode!r}; using sonar")
        return PERPLEXITY_MODE_SONAR
...
def _build_sonar_payload(prompt: str, model: str, date_range: tuple[str, str], config: dict) -> dict:
    ...
    search_mode = _config_text(config, "TRENDPIPE_PERPLEXITY_SEARCH_MODE").lower()
    if search_mode in SONAR_SEARCH_MODES:
        web_options["search_mode"] = search_mode

    domains = _csv_values(_config_text(config, "TRENDPIPE_PERPLEXITY_DOMAIN_FILTER"), limit=20)
    if domains:
        web_options["search_domain_filter"] = domains

    languages = _csv_values(_config_text(config, "TRENDPIPE_PERPLEXITY_LANGUAGE_FILTER"), limit=20)
    if languages:
        web_options["search_language_filter"] = languages

    recency = _config_text(config, "TRENDPIPE_PERPLEXITY_RECENCY_FILTER").lower()
    ...
    effort = _config_text(config, "TRENDPIPE_PERPLEXITY_REASONING_EFFORT").lower()
...
def _build_search_payload(query: str, date_range: tuple[str, str], config: dict) -> dict:
    ...
    payload: dict[str, object] = {
        "query": query,
        "max_results": _positive_int(config.get("TRENDPIPE_PERPLEXITY_MAX_RESULTS"), 10, 1, 20),
    }

    context_size = _config_text(config, "TRENDPIPE_PERPLEXITY_SEARCH_CONTEXT_SIZE").lower()
    if context_size in SEARCH_CONTEXT_SIZES:
        payload["search_context_size"] = context_size

    country = _config_text(config, "TRENDPIPE_PERPLEXITY_COUNTRY").upper()
    if len(country) == 2:
    ...
    # published-date filters are present. trendpipe already passes an exact
...
def _poll_async_sonar(json_data: dict, headers: dict, config: dict) -> tuple[dict, dict]:
    timeout_seconds = _positive_int(
        config.get("TRENDPIPE_PERPLEXITY_DEEP_TIMEOUT_SECONDS"),
