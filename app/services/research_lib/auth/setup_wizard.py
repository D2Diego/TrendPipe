"""First-run setup wizard for trendpipe.
...
_WELCOME_TEXT = """Welcome to /trendpipe! I research any topic across Reddit, X, YouTube, TikTok, Digg, arXiv, Techmeme, HN, Polymarket & more - what people actually said in the last 30 days. Let's get you set up (~30s).
...
def write_setup_config(env_path: Path, from_browser: str | None = None) -> bool:
    ...
    Args:
        env_path: Path to the .env file (e.g. ~/.config/trendpipe/.env)
...
def _device_handle_path() -> Path:
    ...
    return Path(tempfile.gettempdir()) / "trendpipe-github-device-handle.json"
