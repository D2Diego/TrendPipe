"""Scan saved trendpipe research artifacts into a deterministic library."""
...
DEFAULT_MEMORY_DIR = Path.home() / "Documents" / "TrendPipe"
DEFAULT_BRIEFS_DIR = Path.home() / ".local" / "share" / "trendpipe" / "briefs"
LIBRARY_ID_FILENAME = ".trendpipe-library-id"

_REPORT_TITLE = re.compile(r"^#\s+trendpipe(?:\s+v[^:]+)?:\s*(.+?)\s*$", re.MULTILINE | re.IGNORECASE)
...
class LibraryEntry:
    ...
    def entry_id(self) -> str:
        return f"urn:trendpipe:{self.slug}:{self.identity_hash}:{self.published_date.isoformat()}"
