...
from .. import dates, health, relevance, schema
from ..sources import hiring_signals
from . import library_index, skill_meta
from ..pipeline import registers, signals
...
def _render_badge() -> list[str]:
    ...
    return [
        f"🌐 trendpipe v{version} · synced {today}",
        "",
    ]
...
PRIVATE_CORPUS_START = "<!-- TRENDPIPE_PRIVATE_CORPUS_START -->"
PRIVATE_CORPUS_END = "<!-- TRENDPIPE_PRIVATE_CORPUS_END -->"
...
def _render_library_context(report: schema.Report) -> list[str]:
    ...
    lines = [
        ...
        "TRENDPIPE_LIBRARY_CONTEXT=off to hide)._",
        "",
    ]
...
def _defang_corpus_sentinels(value: str) -> str:
    ...
    """
    return value.replace("TRENDPIPE_PRIVATE_CORPUS", "TRENDPIPE_PRIVATE-CORPUS")
...
def render_compact(
    ...
    lines = [
        *_render_badge(),
        f"# trendpipe v{_skill_version()}: {report.topic}",
        "",
...
def _render_canonical_boundary() -> list[str]:
    ...
    return [
        "",
        "---",
        "# END OF trendpipe CANONICAL OUTPUT",
        "",
...
def _render_pre_research_warning(report: schema.Report) -> list[str]:
    ...
    return [
        ...
        f"rerun `/trendpipe {report.topic}`. The skill will resolve handles",
...
def _render_degraded_run_warning(report: schema.Report) -> list[str]:
    ...
    return [
        ...
        "Hermes, Gemini, or any agent runtime that invoked `/trendpipe`): YOU ARE",
...
def render_comparison_multi(
    ...
    *,
    ...
    lines: list[str] = [
        *_render_badge(),
        f"# trendpipe v{_skill_version()}: {synthesized_topic}",
        "",
        *_assistant_safety_lines(),
...
def render_full(report: schema.Report) -> str:
    ...
    # self-describing. The artifact is populated by trendpipe.py's
