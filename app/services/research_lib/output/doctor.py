...
from .. import backends, env, health, http
...
def _host_native_web_note(config: Dict[str, Any]) -> str:
    ...
    Keys on TRENDPIPE_NATIVE_SEARCH (via env.is_native_search) AND on
    CLAUDECODE as a host signal - Claude Code always exposes a web-search tool,
    but `doctor` run in a plain shell never sees the TRENDPIPE_NATIVE_SEARCH
    ...
    if env.is_native_search(config):
        return (
            "host-native search active (TRENDPIPE_NATIVE_SEARCH): the host's "
        ...
        if not record["fix"]:
            if not config.get("SCRAPECREATORS_API_KEY"):
                ...
                record["fix"] = (
                    ...
                    "~/.config/trendpipe/.env to enable YouTube comment text"
                )
    if notes:
        joined = "; ".join(notes)
    ...
    if not has_key:
        return _record(
            ...
            fix=(
                ...
                "~/.config/trendpipe/.env, then add perplexity to INCLUDE_SOURCES"
            ),
        )
...
def _library_record(config):
    ...
    try:
        count = _count_saved_briefs(
            config.get("TRENDPIPE_MEMORY_DIR") or library.DEFAULT_MEMORY_DIR
        )
    except Exception:
        return _record(
            status=health.OK,
    ...
    if count == 0:
        ...
        note = (
            ...
            "(TRENDPIPE_LIBRARY_CONTEXT=off to hide)"
        )
...
# _last_report_cache_path() in trendpipe.py (the same mirror pattern the
# doctor-cache block below already uses for its own schema stamp).
# ---------------------------------------------------------------------------

REPORT_CACHE_SCHEMA_VERSION = "trendpipe-report-cache/v1"
...
def render_text(report: Dict[str, Any]) -> str:
    lines: List[str] = [f"trendpipe doctor — engine v{report['engine_version']}"]
...
def render_postmortem_text(pm: Dict[str, Any]) -> str:
    lines = [f"trendpipe post-mortem — engine v{pm['engine_version']}"]
    if not pm.get("present"):
        lines.append("")
        lines.append(
            "No saved run found - run `/trendpipe <topic>` first, or "
...
# trendpipe.py) and a config fingerprint — a sha256 over the same
...
DOCTOR_CACHE_SCHEMA_VERSION = "trendpipe-doctor-cache/v1"

# TTL in SECONDS (env-tunable via TRENDPIPE_DOCTOR_TTL; registered in
...
def cache_ttl_seconds(config: Dict[str, Any]) -> int:
    """TRENDPIPE_DOCTOR_TTL in seconds; process env > config; default 900."""
    raw: Any = os.environ.get("TRENDPIPE_DOCTOR_TTL")
    if raw is None:
        raw = (config or {}).get("TRENDPIPE_DOCTOR_TTL")
...
def _write_cache(report: Dict[str, Any], config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        sys.stderr.write(
            f"[trendpipe] WARNING: could not write doctor cache: "
...
def probe_timeout_seconds(config: Dict[str, Any]) -> int:
    ...
    raw: Any = os.environ.get("TRENDPIPE_DOCTOR_PROBE_TIMEOUT")
    if raw is None:
        raw = (config or {}).get("TRENDPIPE_DOCTOR_PROBE_TIMEOUT")
    if raw is None or raw == "":
    ...
    *,
    ...
    try:
        req = urllib.request.Request(
            url, headers=headers or {"User-Agent": "trendpipe-doctor"}
        )
...
def run(
    ...
    *,
    ...
    if probe or not fresh_run:
        ...
        sys.stderr.write(
            f"[trendpipe] doctor live probe: checking {len(probeable)} free/CLI "
