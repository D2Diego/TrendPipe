...
def _resolve_model_pins(config: dict[str, Any], depth: str, provider_name: str) -> tuple[str, str, str]:
    ...
    planner_model = config.get("TRENDPIPE_PLANNER_MODEL") or default_planner
    rerank_model = config.get("TRENDPIPE_RERANK_MODEL") or default_rerank
...
def mock_runtime(config: dict[str, Any], depth: str) -> schema.ProviderRuntime:
    ...
    provider_name = (config.get("TRENDPIPE_REASONING_PROVIDER") or "gemini").lower()
...
def resolve_runtime(config: dict[str, Any], depth: str) -> tuple[schema.ProviderRuntime, ReasoningClient | None]:
    ...
    provider_name = (config.get("TRENDPIPE_REASONING_PROVIDER") or "auto").lower()
