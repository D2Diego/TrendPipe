"""Environment and API key management for trendpipe skill."""
...
# Set TRENDPIPE_CONFIG_DIR="" for clean/no-config mode
# Set TRENDPIPE_CONFIG_DIR="/path/to/dir" for custom config location
_config_override = os.environ.get('TRENDPIPE_CONFIG_DIR')
if _config_override == "":
    ...
    CONFIG_DIR = Path.home() / ".config" / "trendpipe"
    CONFIG_FILE = CONFIG_DIR / ".env"
...
# Example: `security add-generic-password -a "$USER" -s trendpipe-XAI_API_KEY -w "xai-..."`.
KEYCHAIN_SERVICE_PREFIX = "trendpipe-"
...
# TRENDPIPE_KEYCHAIN_ALIASES, for example:
...
KEYCHAIN_ALIASES_ENV = "TRENDPIPE_KEYCHAIN_ALIASES"
...
def _project_config_trusted(policy: ConfigLoadPolicy, file_env: dict[str, Any]) -> bool:
    if policy.allow_project_config:
        return True
    process_value = os.environ.get("TRENDPIPE_TRUST_PROJECT_CONFIG")
    if process_value is not None:
        return _truthy(process_value)
    return _truthy(file_env.get("TRENDPIPE_TRUST_PROJECT_CONFIG"))
...
def _check_file_permissions(path: Path) -> None:
    ...
    try:
        ...
        if mode & 0o044:
            sys.stderr.write(
                f"[trendpipe] WARNING: {path} is readable by other users. "
                f"Run: chmod 600 {path}\n"
            )
            sys.stderr.flush()
    except OSError as exc:
        sys.stderr.write(f"[trendpipe] WARNING: could not stat {path}: {exc}\n")
        sys.stderr.flush()
...
def _parse_keychain_aliases(raw: str | None) -> dict[str, list[dict[str, str]]]:
    ...
    `trendpipe-<KEY>` lookups; malformed JSON emits a warning.
    """
    if not raw:
        return {}
    try:
        ...
        sys.stderr.write(
            f"[trendpipe] WARNING: {KEYCHAIN_ALIASES_ENV} is not valid JSON; "
...
def _load_keychain(keys: list[str], aliases: dict[str, list[dict[str, str]]] | None = None) -> dict[str, str]:
    ...
    ``TRENDPIPE_KEYCHAIN_ALIASES``. Lookup failures are silent — Keychain is
...
def _load_pass(keys: list[str], prefix: str) -> dict[str, str]:
    ...
    ``trendpipe-<key>`` service-name convention — so any user stores keys under
    that namespace without editing code (prefix overridable via
    ``TRENDPIPE_PASS_PREFIX``). The secret is decrypted in a subprocess and
...
def _find_project_env() -> Path | None:
    ...
    Searches for .claude/trendpipe.env in each parent directory,
    stopping at the git root, user's home directory, or filesystem root.
    """
    cwd = Path.cwd()
    for parent in [cwd, *cwd.parents]:
        candidate = parent / '.claude' / 'trendpipe.env'
        if candidate.exists():
            return candidate
...
def get_config(policy: ConfigLoadPolicy | None = None) -> dict[str, Any]:
    ...
    Priority (highest wins):
      ...
      2. Trusted .claude/trendpipe.env (per-project config)
      3. ~/.config/trendpipe/.env (global config)
      4. macOS Keychain items prefixed ``trendpipe-`` (Darwin only)
    """
    ...
    # (not import time, so a .env-set TRENDPIPE_PASS_PREFIX is honored), and
    ...
    pass_prefix = (
        os.environ.get("TRENDPIPE_PASS_PREFIX")
        or merged_env.get("TRENDPIPE_PASS_PREFIX")
    ...
    keys = [
        ...
        ('TRENDPIPE_DEBUG', None),
        ('XAI_API_KEY', None),
        ('GOOGLE_API_KEY', None),
        ('GEMINI_API_KEY', None),
        ('GOOGLE_GENAI_API_KEY', None),
        ('XIAOHONGSHU_API_BASE', None),
        ('TRENDPIPE_REASONING_PROVIDER', 'auto'),
        ('TRENDPIPE_PLANNER_MODEL', None),
        ('TRENDPIPE_RERANK_MODEL', None),
        ('TRENDPIPE_X_MODEL', None),
        ('TRENDPIPE_X_BACKEND', None),
        ('TRENDPIPE_REDDIT_BACKEND', None),
        # Doctor cache freshness window in seconds (doctor --cached).
        ('TRENDPIPE_DOCTOR_TTL', None),
        # Per-source deadline (seconds) for doctor --probe live checks.
        ('TRENDPIPE_DOCTOR_PROBE_TIMEOUT', None),
        ('TRENDPIPE_REDDIT_SC_MIN_ITEMS', None),
        ('TRENDPIPE_STORE', None),
        ...
        ('TRENDPIPE_DISCOVERY_QUEUE', None),
        ...
        ('TRENDPIPE_CORPUS_DIRS', None),
        # Corpus evidence is omitted from the stable agent JSON export unless
        # this explicit privacy opt-in is truthy.
        ('TRENDPIPE_CORPUS_IN_EXPORT', None),
        ('TRENDPIPE_LIBRARY_OWNER', None),
        ('TRENDPIPE_LIBRARY_CONTEXT', 'on'),
        ('TRENDPIPE_PUBLISH_PASSWORD', None),
        ...
        ('TRENDPIPE_PERPLEXITY_MODE', 'sonar'),
        ('TRENDPIPE_PERPLEXITY_MODEL', None),
        ('TRENDPIPE_PERPLEXITY_MAX_RESULTS', None),
        ('TRENDPIPE_PERPLEXITY_SEARCH_CONTEXT_SIZE', None),
        ('TRENDPIPE_PERPLEXITY_SEARCH_MODE', None),
        ('TRENDPIPE_PERPLEXITY_DOMAIN_FILTER', None),
        ('TRENDPIPE_PERPLEXITY_LANGUAGE_FILTER', None),
        ('TRENDPIPE_PERPLEXITY_COUNTRY', None),
        ('TRENDPIPE_PERPLEXITY_RECENCY_FILTER', None),
        ('TRENDPIPE_PERPLEXITY_REASONING_EFFORT', None),
        ('TRENDPIPE_PERPLEXITY_DEEP_TIMEOUT_SECONDS', '600'),
        ...
        ('TRENDPIPE_TRUSTPILOT_NO_BROWSER', None),
        ('FROM_BROWSER', None),
        ('TRENDPIPE_TRUST_PROJECT_CONFIG', None),
        ('SETUP_COMPLETE', None),
        ('INCLUDE_SOURCES', ''),
        ('EXCLUDE_SOURCES', ''),
        ('TRENDPIPE_DEFAULT_SEARCH', ''),
        # Resolve the user-facing default in trendpipe.py so an absent value
        ...
        ('TRENDPIPE_REGISTER', None),
        ('FUN_LEVEL', 'medium'),
        # Backward compatibility for configs written by the original `eli5 on`
        # follow-up command. New writes use TRENDPIPE_REGISTER=eli5.
        ('ELI5_MODE', None),
        ('TRENDPIPE_YOUTUBE_SSH_HOST', None),
        ('TRENDPIPE_REPORT_CACHE_TTL_SECONDS', None),
        ('TRENDPIPE_VERIFY_FRESHNESS', None),
        ('TRENDPIPE_TRANSCRIPT_TIMEOUT', None),
        ...
        ('GROQ_API_KEY', None),
        ('TRENDPIPE_YT_SUB_LANGS', 'en,es,pt'),
        ('TRENDPIPE_YT_TRANSCRIPT_FAST_TIMEOUT', None),
        ('TRENDPIPE_YT_SEARCH_TIMEOUT', None),
        ('GITHUB_TOKEN', None),
    ]
    ...
    if config.get('TRENDPIPE_DEBUG'):
        os.environ.setdefault('TRENDPIPE_DEBUG', config['TRENDPIPE_DEBUG'])
    ...
    for key in (
        'TRENDPIPE_YT_SUB_LANGS',
        'TRENDPIPE_YT_TRANSCRIPT_FAST_TIMEOUT',
        'TRENDPIPE_YT_SEARCH_TIMEOUT',
    ):
        if config.get(key):
...
def cookie_extraction_browsers(config: dict[str, Any]) -> list[str]:
    ...
    if "," in from_browser:
        ...
        if unknown:
            sys.stderr.write(
                "[trendpipe] WARNING: FROM_BROWSER ignored unrecognized browser(s): "
    ...
    sys.stderr.write(
        f"[trendpipe] WARNING: FROM_BROWSER='{from_browser}' is not a recognized "
...
X_BACKEND_PIN_VAR = 'TRENDPIPE_X_BACKEND'
REDDIT_BACKEND_PIN_VAR = 'TRENDPIPE_REDDIT_BACKEND'
REDDIT_SC_MIN_ITEMS_VAR = 'TRENDPIPE_REDDIT_SC_MIN_ITEMS'
...
def x_backend_chain(config: dict[str, Any], local_only: bool = False) -> list[str]:
    ...
    A ``TRENDPIPE_X_BACKEND`` pin forces a single backend (no failover): the
...
def is_native_search(config: dict[str, Any]) -> bool:
    ...
    ``TRENDPIPE_NATIVE_SEARCH`` when the runtime actually has a native web-search
    ...
    """
    raw = config.get('TRENDPIPE_NATIVE_SEARCH')
    if raw is None:
        return False
...
def is_xiaohongshu_available(config: dict[str, Any]) -> bool:
    ...
    for base in _xiaohongshu_api_base_candidates(config):
        try:
            ...
            sys.stderr.write(
                f"[trendpipe] WARNING: unexpected error checking Xiaohongshu "
