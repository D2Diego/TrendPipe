...
def _load_task_history_helpers():
    """
    From WebUI Isolation loading in the entrance is not dependent Streamlit task history pure function.

    Direct Import Main.py You can perform a full page rendering. Test to compile target constants and functions only, both authenticating
    The real realization of the merger also avoids the redismantling of a production module with a small number of functions for unit testing.
    """
...
def test_find_final_task_video_ignores_intermediate_files(tmp_path):
    """Mission history can only make final Synthetic intermediate files cannot be used because the pieces are identified as complete."""
    for file_name in (
        "combined-1.mp4",
        "temp-clip-1.mp4",
...
def test_find_final_task_video_returns_first_numbered_output(tmp_path):
    """Multiple pieces of tasks are consistent with running-time results, and the final video with the smallest serial number is defaulted."""
...
def test_restore_requirements_require_file_in_upload_voice_mode():
    """Reactivate the uploading of a voice, the audio file must be re-selected to continue the upload mode."""
    requirements = build_restore_upload_requirements(
        {
            "video_source": "pexels",
            "custom_audio_file": "/old-task/custom-audio.wav",
...
def test_restore_requirements_allow_replacing_upload_with_other_voice_modes():
    """When the user voluntarily switches to automatic or no voice, the historical file is no longer forced to be restored."""
    requirements = build_restore_upload_requirements(
        {
            "video_source": "pexels",
            "custom_audio_file": "/old-task/custom-audio.wav",
