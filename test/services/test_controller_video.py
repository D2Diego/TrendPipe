...
class TestVideoControllerHelpers(unittest.TestCase):
    ...
    def test_sanitize_upload_filename_removes_client_path(self):
        """Windows and POSIX The client path can only keep the last secure file name."""
    ...
    def test_fastapi_startup_recovers_interrupted_cross_posts(self):
        """API A residual recovery must be implemented at the start of the process."""
    ...
    def test_sanitize_upload_filename_rejects_empty_name(self):
        """Empty filename and directory placeholder cannot enter the service-end storage path."""
        for filename in ("", ".", "..", "/"):
            with self.subTest(filename=filename):
    ...
    def test_parse_byte_range_supports_common_player_requests(self):
        """Accurate boundaries should be obtained for the closed, open and suffocating spaces common to players."""
        cases = (
            (None, (0, 9)),
            ("bytes=2-5", (2, 5)),
            ("bytes=4-", (4, 9)),
...
class TestVideoControllerTasks(unittest.TestCase):
    ...
    def test_create_task_queues_requested_pipeline_stage(self):
        """The creation task should be sustained in its initial state and the original request model and cessation phase should be submitted to the queue."""
        body = MagicMock()
    ...
    def test_get_all_tasks_preserves_pagination(self):
        """The task list response must contain the total number of returns from the status layer and the requested page break parameters."""
        with patch.object(
    ...
    def test_task_query_preserves_structured_failure_details(self):
        """The failure phase and the error information must be returned through the task query interface."""
        failed_task = {
            "task_id": "failed-task",
    ...
    def test_task_query_schema_documents_success_and_failure_states(self):
        """OpenAPI Model examples must cover both the state of release success and the state of generation failure."""
    ...
    def test_delete_allows_completed_task(self):
        """The original deletion should be maintained for normally completed tasks."""
        completed_task = {
        ...
        with patch.object(
            ...
            "task_dir",
            return_value="/tmp/trendpipe-completed-task-test",
        ), patch.object(
...
class TestVideoControllerFiles(unittest.TestCase):
    ...
    def test_upload_video_material_validates_complete_extension(self):
        """The uppercase legal extension should be accepted and the no-point pseudo-extension rejected."""
    ...
    def test_stream_video_returns_requested_bytes(self):
        """Range Response body and text Content-Range It must be consistent with the calculations."""
    ...
    def test_download_video_uses_resolved_file(self):
        """Download responses should use the real path and original filename of the white-list directory."""
        with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # macOS Yes. /var Yes. /private/var , the security resolution returns the real path.
