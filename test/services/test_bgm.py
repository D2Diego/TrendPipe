...
class TestBackgroundMusicService(unittest.TestCase):
    def test_should_use_bgm_is_provider_independent(self):
        """The universal switch must cover current sources and future providers and no longer be able to write a special vendor judgement."""
        test_cases = [
            ("random", 0.2, True),
            ("custom", 0.2, True),
            ("sonilo", 0.2, True),
    ...
    def test_sanitize_upload_filename_strips_directory_components(self):
        self.assertEqual(
            bgm.sanitize_upload_filename("../../User Music.mp3"), "User Music.mp3"
        )
        self.assertEqual(
    ...
    def test_sanitize_upload_filename_accepts_common_audio_formats(self):
        # WebUI and API Share white lists in the same format; cover case and common containers to avoid future
        # Changes to upload controls only support interface displays and service levels reject files.
    ...
    def test_validate_bgm_upload_checks_audio_without_persisting_file(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # WebUI Precheck allows only temporary files to be used for a short time and cannot be used when the user has not clicked to generate the file
            # Stay with Enduring BGM Nor can the directory change the document pointer required for the follow-up audition.
    ...
    def test_validate_bgm_upload_rejects_invalid_audio_and_cleans_temp_file(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # The failed pre-screening also does not leave a temporary audio in the DOP directory to avoid subsequent randomization BGM
            # The login selection also avoids the accumulation of invalid files over time.
            self.assertEqual(os.listdir(temp_dir), [])
    ...
    def test_save_bgm_upload_rejects_invalid_audio_before_replacing_target(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # The verification failed before the atom was replaced and the existing user files could not be damaged.
