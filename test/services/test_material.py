...
class TestMaterialTlsVerification(unittest.TestCase):
    ...
    def test_search_pexels_uses_tls_verification_by_default(self):
        """
        Default path must be opened TLS Verify, avoid material. API key And the material to return. URL
        Intermediaries attack and intercept or tamper with public networks or untrustworthy proxy environments.
        """
    ...
    def test_search_pixabay_allows_explicit_tls_disable_for_proxy(self):
        """
        A few business agents use their own visa. The scene must be clearly configured to close TLS Check it out.
    ...
    def test_remote_searches_only_return_requested_orientation(self):
        """
        All three material sources must return only to the material in the direction of the target and avoid the task of embedding the material behind the screen.
        Pass. letterbox A clear black side.Pexels Use remote parameters and verify them locally.
        Pixabay and Coverr Use response size for local filtering.
        """
        config.app["pexels_api_keys"] = ["pexels-key"]
        config.app["pixabay_api_keys"] = ["pixabay-key"]
    ...
    def test_video_aspect_matching_rejects_unknown_dimensions(self):
        """The material for which direction cannot be confirmed cannot be entered in the strict stand-by list."""
        self.assertTrue(
    ...
    def test_coverr_passes_orientation_filter_to_remote_search(self):
        """Coverr Cross-screen searches should be screened at the service end, and square material continues to be verified with local dimensions."""
        config.app["coverr_api_keys"] = ["coverr-key"]
        config.proxy.clear()
    ...
    def test_square_search_preserves_crop_compatible_materials(self):
        """
        Pixabay and Coverr Original video is rarely available. The square output must continue to accept the tailorable.
        Screen material, otherwise an empty list is available directly at the search stage when both sources are selected.
        """
        config.app["pixabay_api_keys"] = ["pixabay-key"]
        config.app["coverr_api_keys"] = ["coverr-key"]
        config.proxy.clear()
    ...
    def test_search_pixabay_reports_api_rate_limit(self):
        """
        Pixabay My own. 429 Limited flow and Cloudflare HTML Challenge It's different.
        Reservations Retry-After It helps users to judge when to try again without recording the response body.
        """
        config.app["pixabay_api_keys"] = ["pixabay-key"]
        config.proxy.clear()
    ...
    def test_material_source_record_uses_public_whitelist(self):
        """
        The list of tasks should contain only publicly reversible fields and not write signature parameters, download address,
        An additional field or the absolute path of the machine to which the caller transfers.
        """
    ...
    def test_download_videos_can_round_robin_terms_in_script_order(self):
        """
        Once you start matching the material in the order of the case, you can't let the multiple candidates for the first key word first.
        Audio time fill. There are multiple candidates for the simulation of the two keywords.
        term1-I don't think so.1 Yeah.term2-I don't think so.1 Yeah.term1-I don't think so.2 Yeah, close to the script.
        """
        search_results = {
            "opening city": [
    ...
    def test_material_source_persistence_failure_does_not_break_download(self):
        """When the supporting mission record fails, the successful material that has already been downloaded should be returned normally to the master process."""
        item = material.MaterialInfo(
            provider="pexels",
            url="https://v.example/a1.mp4",
            duration=5,
...
class TestCoverrProvider(unittest.TestCase):
    ...
    def test_search_coverr_uses_mp4_download_url(self):
        ...
        # url Fields are mp4_download URL,Not anymore. coverr://id|url Encoded
        self.assertEqual(
    ...
    def test_search_coverr_uses_tls_verification_by_default(self):
        """and pexels/pixabay Consistent:When not visible TLS Verify default on."""
        config.app["coverr_api_keys"] = ["coverr-key"]
        config.app.pop("tls_verify", None)
        config.proxy.clear()
    ...
    def test_search_coverr_allows_explicit_tls_disable_for_proxy(self):
        """The company has to be able to shut down the visa agency. TLS Verify."""
        config.app["coverr_api_keys"] = ["coverr-key"]
        config.app["tls_verify"] = False
        config.proxy.clear()
    ...
    def test_search_coverr_filters_by_min_duration_and_accepts_string(self):
        """
        Coverr duration Fields may be in different responses number or string,
        Both formats are accepted.;Less than minimum_duration It should be filtered.
        """
        config.app["coverr_api_keys"] = ["coverr-key"]
        config.app.pop("tls_verify", None)
        config.proxy.clear()
    ...
    def test_search_coverr_skips_invalid_items(self):
        """Missing id or missing urls.mp4_download entry should be skipped,Shouldn't leave the anomaly."""
        config.app["coverr_api_keys"] = ["coverr-key"]
        config.app.pop("tls_verify", None)
        config.proxy.clear()
    ...
    def test_search_coverr_returns_empty_on_failure(self):
        """
        Response structure abnormal. / When the network is abnormal,Function must return [] Instead of dropping the anomaly.,
        and pexels/pixabay Be consistent.
        """
        config.app["coverr_api_keys"] = ["coverr-key"]
        config.app.pop("tls_verify", None)
        config.proxy.clear()
    ...
    def test_download_videos_passes_mp4_download_url_to_save_video(self):
        """
        Yes. source="coverr" Time:
          1. dispatch Present. search_videos_coverr
          2. coverr item Take Universal Download Path:save_video Here's what I got. mp4_download URL
             (Not anymore. coverr://id|url Encoded,No more calls. PATCH ping)
          3. Return Save Path
        """
        config.app["coverr_api_keys"] = ["coverr-key"]
        config.app.pop("tls_verify", None)
        ...
        # 2. save_video Here's what I got. mp4_download URL,Organisation
