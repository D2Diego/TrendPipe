...
class TestCli(unittest.TestCase):
    ...
    def test_complete_script_can_replace_video_subject(self):
        ...
        self.assertEqual(params.video_script, "Full video file")
    ...
    def test_build_video_params_with_local_materials(self):
        ...
        self.assertEqual(params.video_subject, "Test Theme")
    ...
    def test_run_cli_dispatches_task_start(self):
        with patch("app.services.task.start", return_value={"script": "ok"}) as start, patch(
            ...
            code = cli.run_cli(["--video-subject", "Command Line Test", "--stop-at", "script"])
        ...
        self.assertEqual(kwargs["params"].video_subject, "Command Line Test")
    ...
    def test_run_cli_returns_error_when_task_fails(self):
        with patch("app.services.task.start", return_value=None), patch(
            ...
            code = cli.run_cli(["--video-subject", "Failed scene"])

        self.assertEqual(code, 1)
        log_error.assert_called_once()

    def test_run_cli_returns_error_for_structured_task_failure(self):
        """The mission service returns the failed structured message.CLI The withdrawal must remain non-zero."""
        failure = {
    ...
    def test_positive_volume_custom_bgm_requires_file_before_task_start(self):
        """Enable custom BGM The lack of documentation must still be reported before the start of the mission."""
        with (
    ...
    def test_zero_volume_custom_bgm_skips_file_requirement_and_resolution(self):
        """0 Volume should ignore missing or invalid files, and WebUI It's consistent with video services."""
    ...
    def test_custom_bgm_reports_service_resolution_failure_before_task_start(self):
        """Illegal format or cross-border route should be converted to include a uniform format range CLI Error."""
        from app.services import bgm as bgm_service
    ...
    def test_help_does_not_initialize_application_or_write_logs(self):
        """The help order should be loaded independently of the business configuration and be user-friendly and script-gathering."""
        ...
        self.assertIn("Generate TrendPipe videos", result.stdout)
