"""Remote API client for trendpipe (optional hosted-backend mode).

When both TRENDPIPE_API_KEY and TRENDPIPE_API_BASE are set, the engine
...
endpoint comes only from TRENDPIPE_API_BASE - there is no built-in default.
...
def _api_base() -> str:
    ...
    # mode is gated on this being set (see trendpipe.py), so by the time this
    # is called it is populated; an empty value means "not configured".
    return (os.environ.get("TRENDPIPE_API_BASE") or "").rstrip("/")
...
def _auth_headers() -> dict[str, str]:
    ...
    key = env.read_secret_env("TRENDPIPE_API_KEY") or ""
...
def _handle_http_error(exc: http.HTTPError) -> int:
    ...
    if exc.status_code == 401:
        _err(
            ...
            "TRENDPIPE_API_KEY (and TRENDPIPE_API_BASE), or unset them "
...
def _handle_clarify(resp: dict) -> int:
    ...
    sys.stderr.flush()
    _err(
        "No search was started. Re-run trendpipe with the chosen angle "
...
def _render_complete(row: dict, topic: str, emit: str, save_dir, save_suffix: str) -> int:
    ...
    if save_dir:
        ...
        sys.stderr.write(f"[trendpipe] Saved output to {out_path}\n")
        sys.stderr.flush()
    print(rendered)
    return 0
...
def run_hosted(
    ...
    *,
    ...
    _err(f"Running via trendpipe API ({_api_base()}), depth={depth}")
    try:
