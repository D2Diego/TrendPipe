"""Trustpilot brand-sentiment source for trendpipe.
...
Default-on safety (three gates):
  ...
  2. Browser opt-out. Automated contexts (cron, CI, the eval harness) can set
     ``TRENDPIPE_TRUSTPILOT_NO_BROWSER`` to disable the source entirely, so a
...
NO_BROWSER_ENV = "TRENDPIPE_TRUSTPILOT_NO_BROWSER"
...
def _harvest_allowed(config: Optional[Dict[str, Any]]) -> bool:
    ...
    config (e.g. a bare ``TRENDPIPE_TRUSTPILOT_NO_BROWSER=1`` in cron/CI).
    """
