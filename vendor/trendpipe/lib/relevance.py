...
def token_overlap_relevance(
    ...
    if normalized_query:
        ...
        if not contained and cjk.has_cjk(normalized_query):
            # CJK has no inter-word spaces, so a multi-token query may never
            # appear verbatim in continuous source text. Retry containment
            # without spaces so the phrase bonus remains available for CJK.
