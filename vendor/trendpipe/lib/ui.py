"""Terminal UI utilities for trendpipe skill."""
...
MINI_BANNER = f"""{Colors.PURPLE}{Colors.BOLD}/trendpipe{Colors.RESET} {Colors.DIM}· researching...{Colors.RESET}"""
...
To fix this:
1. Add AUTH_TOKEN and CT0 to ~/.config/trendpipe/.env, or to trusted .claude/trendpipe.env with TRENDPIPE_TRUST_PROJECT_CONFIG=1
...
class ProgressDisplay:
    ...
    def _show_banner(self):
        if IS_TTY:
            ...
            sys.stderr.write(f"/trendpipe · researching: {self.topic}\n")
        sys.stderr.flush()
...
def show_diagnostic_banner(diag: dict):
    ...
    if IS_TTY:
        ...
        _header = f"/trendpipe v{_skill_version()} - Source Status"
        ...
        lines.append(f"{Colors.DIM}│{Colors.RESET}                                                     {Colors.DIM}│{Colors.RESET}")
        lines.append(f"{Colors.DIM}│{Colors.RESET}  Config: {Colors.BOLD}~/.config/trendpipe/.env{Colors.RESET}                  {Colors.DIM}│{Colors.RESET}")
        lines.append(f"{Colors.DIM}└─────────────────────────────────────────────────────┘{Colors.RESET}")
    else:
        # Plain text for non-TTY (Claude Code / Codex)
        lines.append("┌─────────────────────────────────────────────────────┐")
        _header_plain = f"/trendpipe v{_skill_version()} - Source Status"
        ...
        lines.append("│                                                     │")
        lines.append("│  Config: ~/.config/trendpipe/.env                  │")
        lines.append("└─────────────────────────────────────────────────────┘")
