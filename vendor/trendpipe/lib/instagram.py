"""Instagram Reels search via ScrapeCreators API for /trendpipe.
...
# so the default is generous; override via TRENDPIPE_TRANSCRIPT_TIMEOUT.
...
def _resolve_transcript_timeout(
    ...
    Priority (highest wins):
      ...
      2. ``TRENDPIPE_TRANSCRIPT_TIMEOUT`` in os.environ
      3. ``TRENDPIPE_TRANSCRIPT_TIMEOUT`` in caller-supplied config dict
      4. ``DEFAULT_TRANSCRIPT_TIMEOUT`` (30s)

    Mirrors the ``os.environ.get(X) or config.get(X)`` pattern used for
    TRENDPIPE_STORE in trendpipe.py so the env var works whether it's
    shell-exported or set in ~/.config/trendpipe/.env.
    """
    if timeout is not None:
        try:
            ...
            pass
    raw = os.environ.get("TRENDPIPE_TRANSCRIPT_TIMEOUT")
    if not raw and config:
        raw = config.get("TRENDPIPE_TRANSCRIPT_TIMEOUT")
    if raw:
        try:
            return float(raw)
        except (TypeError, ValueError):
...
def fetch_captions(
    ...
    Args:
        ...
        timeout: Optional per-request transcript timeout in seconds. When
            None, resolves from TRENDPIPE_TRANSCRIPT_TIMEOUT (env or
            config), defaulting to DEFAULT_TRANSCRIPT_TIMEOUT (30s).
        config: Optional config dict (from env.get_config()) used as a
            fallback source for TRENDPIPE_TRANSCRIPT_TIMEOUT when the
